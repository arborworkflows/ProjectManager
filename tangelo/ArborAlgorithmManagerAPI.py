...
class ArborAlgorithmManager:
    ...
    def runAlgorithmByName(self,algoString, databasename='arbor',projectname='none',treename='none',matrixname='none',charactername='none',outname='none'):
        ...
        results = self.arbor_algorithm_dictionary[str(algoString)](databasename,projectname,treename,matrixname,charactername,outname)
    ...
    def dataIntegrator(self,dbname,proj,tree,matrix,chars,outname):
        ...
        output_tree_collection = self.projectapi.returnCollectionForObjectByName(proj,'PhyloTree',str(outname))
        results = data_integrator_algorithm1.DataIntegratorBySeparateConnection('localhost',dbname,27017,tree_collection, matrix_collection, output_tree_collection, verbose=True)
    ...
    def fitContinuous(self,dbname,proj,tree,matrix,chars,outname,algo_model_parameter='"OU"'):
        ...
        matrix_collection = self.projectapi.returnCollectionForObjectByName(proj,'CharacterMatrix',matrix)
        output_tree_collection = self.projectapi.returnCollectionForObjectByName(proj,'PhyloTree',str(outname))
        results = fit_continuous_geiger.FitContinuousBySeparateConnection('localhost',dbname,27017,tree_collection, matrix_collection, str(chars), algo_model_parameter,output_tree_collection, verbose=True)
        description= str('generated by fitContinuous from ')+str(tree)
        self.projectapi.newTreeInProjectFromExistingCollection(str(outname),proj,description)
        return results
    ...
    def phylogeneticSignal(self,dbname,proj,tree,matrix,character,outname):
        ...
        results = picante_phylogenetic_signal_algorithm.CalculatePhylogeneticSignalBySeparateConnection('localhost',dbname,27017,tree_collection, matrix_collection, str(character), verbose=False)
        return results
    ...
    def speciation(self,dbname,proj,tree,matrix,character,outname):
        ...
        matrix_collection = self.projectapi.returnCollectionForObjectByName(proj,'CharacterMatrix',matrix)
        description= str('generated by phylogeneticSignal from ')+str(tree)
        results = speciation_algorithm.CalculateSpeciationBySeparateConnection('localhost',dbname,27017,tree_collection, matrix_collection, str(character), verbose=False)
        return results
